...
function changeBg(status) {
    if (status === 'Clouds') {
        ...
        document.body.style.backgroundImage = 'url(img/rainy.avif)';
    } else if (status === 'Clear') {
        document.body.style.backgroundImage = 'url(img/clear.jpg)';
    }
    else if (status === 'Snow') {
        document.body.style.backgroundImage = 'url(img/snow.avif)';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchWeather(city) {
        try {
            ...
            if (response.ok) {
                ...
                weatherBody.innerHTML = `<p>${data.message}</p>`;
            }
        } catch (error) {
            console.error('Error fetching weather:', error);
            weatherBody.innerHTML = `<p>An error occurred while fetching weather data.</p>`;
        }
    }

    // Handle city input for real-time weather
    ...
    // Handle form submission for weather prediction
    predictionForm.addEventListener('submit', async (event) => {
        event.preventDefault();

        // Get user input (historical temperature data)
        const historicalData = document.getElementById('historicalData').value
            .split(',')
            .map(temp => parseFloat(temp.trim()));

        if (historicalData.some(isNaN)) {
            alert('Please enter valid comma-separated numerical values.');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                predictionResult.textContent = `Predicted Temperatures: ${data.predictions.join(', ')}`;
            } else {
                predictionResult.textContent = `Error: ${data.message}`;
            }
        } catch (error) {
            console.error('Error fetching predictions:', error);
            predictionResult.textContent = 'An error occurred while fetching predictions.';
        }
    });
});
